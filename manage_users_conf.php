...
if (isset($_POST['Add_fingerID'])) {
    ...
    if ($fingerid == 0) {
        echo "Саусақ ізі ID енгізіңіз!";
        exit();
    }
    if ($dev_uid == 0) {
        echo "Пайдаланушы бөлімін таңдаңыз!";
        exit();
    }
    else{
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                      ...
                      exit();
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            echo "Сіз бір уақытта бірнеше ID қоса алмайсыз";
                        }
                    }
                }
                else{
                    echo "Бұл ID бұрыннан бар! Оны сканерден өшіріңіз";
                    exit();
                }
            }
        }
        else{
            echo "Саусақ ізі ID 1-ден 127-ге дейін болуы керек";
            exit();
        }
    }
}
      ...
      exit();
        ...
        if ($row = mysqli_fetch_assoc($resultl)) {
            ...
            if ($row['username'] == "None") {
                ...
                echo "Бұл саусақ ізі бұрыннан бар";
                exit();
            }
        }
        else {
            echo "Таңдалған саусақ ізі жоқ!";
            exit();
        }
    }
}
          ...
          exit();
            ...
            if ($row = mysqli_fetch_assoc($resultl)) {
                ...
                if ($row['add_fingerid'] == 1) {
                    echo "Біріншіден, сіз пайдаланушыны қосуыңыз керек!";
                    exit();
                }
                else{
                    if (empty($Uname) && empty($Number)) {
                        echo "Бос орынды толтырыңыз";
                        exit();
}
                    else{
                        //check if there any user had already the Serial Number
                ...
                echo "Жаңарту үшін таңдалған пайдаланушы жоқ!";
                exit();
            }
        }
    }
