$(document).ready(function(){
    ...
    $.ajax({
      ...
      success: function(response){
        if (response == "1") {
          ...
          $('#alert').fadeIn(500);
          $('#alert').text("Жаңа пайдаланушы қосылды!");
        }
        else{
          $('#alert').fadeIn(500);
          $('#alert').text(response);
    ...
    $.ajax({
      ...
      success: function(response){
        if (response == "1") {
          ...
          $('#alert').fadeIn(500);
          $('#alert').text("Пайдаланушы жаңартылды!");
        }
        else{
          $('#alert').fadeIn(500);
          $('#alert').text(response);
  ...
  $(document).on('click', '.user_rmo', function(){
    ...
    if (dev_uid === "") {
      $('#alert').fadeIn(500);
      $('#alert').text("Өшіру үшін пайдаланушыны таңдаңыз!!");
      ...
      bootbox.confirm("Бұл пайдаланушыны өшіресіз бе?", function(result) {
        if(result){
          $.ajax({
            url: 'manage_users_conf.php',
            type: 'POST',
            ...
            success: function(response){
              if (response == "1") {
                ...
                $('#alert').fadeIn(500);
                $('#alert').text("Пайдаланушы саусақ ізі өшірілді");
              }
              else{
                $('#alert').fadeIn(500);
                $('#alert').text(response);
  ...
  $(document).on('click', '.select_btn', function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        $('#alert').fadeIn(500);
        $('#alert').text('Пайдаланушы саусақ ізі');
